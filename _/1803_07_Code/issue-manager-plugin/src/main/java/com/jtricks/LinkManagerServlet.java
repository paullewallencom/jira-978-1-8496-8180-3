...
public class LinkManagerServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		issueLinkTypeManager = ComponentManager.getInstance().getComponentInstanceOfType(IssueLinkTypeManager.class);
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		IssueLinkType linkType2 = issueLinkTypeManager.getIssueLinkTypesByName("Enabler").iterator().next();
		try {
	...
	private void deletelink(MutableIssue issue1, MutableIssue issue2, IssueLinkType linkType, User user) throws RemoveException {
	...
	private void printLinks(MutableIssue issue, PrintWriter out) {
		...
		for (IssueLink issueLink : links) {
			out.println(issueLink.getIssueLinkType().getName()+": Linked from "+issueLink.getSourceObject().getKey());
		}
		
		out.println("Outward links!");
		links = issueLinkManager.getOutwardLinks(issue.getId());
		for (IssueLink issueLink : links) {
			out.println(issueLink.getIssueLinkType().getName()+": Linked to "+issueLink.getDestinationObject().getKey());
		}
	}

	private void createLink(MutableIssue issue1, MutableIssue issue2, IssueLinkType linkType, User user) throws CreateException {
