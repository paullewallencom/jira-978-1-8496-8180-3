...
public class WorklogManagerServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		worklogService = ComponentManager.getInstance().getComponentInstanceOfType(WorklogService.class);
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		issue.setOriginalEstimate(7200L);
		issue.store();
		String key = issue.getKey();
		
		// Spent an hour and adjust automatically - Original 2h, Spent 1h, Remaining 1h
		out.println("Logging 1h...");
		Worklog worklog = createWorklog(out, getRefreshedIssue(key), "1h", user);
		// Spent 30m, change remaining estimate to 2h - Original 2h, Spent 1h 30m , Remaining 2h
		...
		// Spent 1h, Retain remaining estimate as 2h - Original 2h, Spent 2h, 30m , Remaining 2h
		...
		// Spent 1h and reduce remaining estimate by 30m - Original 2h, Spent 3h, 30m , Remaining 1h 30m
		...
		// Instead of the auto 90 minutes, let us make it 120 spent & 120 minutes remaining! - Original 2h, Spent 4h 30m , Remaining 2h
	...
	private void deleteWorklog(MutableIssue refreshedIssue, Worklog worklog, User user) {
	...
	private Worklog createWorklogRetainingEstimate(PrintWriter out, MutableIssue issue, String timeSpent, User user) {
		JiraServiceContext jiraServiceContext = new JiraServiceContextImpl(user);
		final WorklogInputParametersImpl.Builder builder = WorklogInputParametersImpl.issue(issue).timeSpent(timeSpent)
				.startDate(new Date()).comment(null).groupLevel(null).roleLevelId(null);
	...
	private Worklog updateWorklogWithNewRemainingEstimate(MutableIssue issue2, User user, Worklog worklog,
		...
		final WorklogInputParametersImpl.Builder builder = WorklogInputParametersImpl.timeSpent(timeSpent)
				.worklogId(worklog.getId()).startDate(new Date()).comment(null).groupLevel(null).roleLevelId(null);
		...
		Worklog updatedLog = this.worklogService
				.updateWithNewRemainingEstimate(jiraServiceContext, (WorklogNewEstimateResult) result, false);
		return updatedLog;
	}

	private Worklog updateWorklog(MutableIssue issue, User user, Worklog worklog, String timeSpent) {
		JiraServiceContext jiraServiceContext = new JiraServiceContextImpl(user);
		...
		Worklog updatedLog = this.worklogService
				.updateAndAutoAdjustRemainingEstimate(jiraServiceContext, result, false);
		return updatedLog;
	}

	private Worklog createWorklog(PrintWriter out, MutableIssue issue, String timeSpent, User user) {
		JiraServiceContext jiraServiceContext = new JiraServiceContextImpl(user);
		final WorklogInputParametersImpl.Builder builder = WorklogInputParametersImpl.issue(issue).timeSpent(timeSpent)
				.startDate(new Date()).comment(null).groupLevel(null).roleLevelId(null);
